...
// ===== Widget de reservas Lanzo (embebido, la clienta no sale de la web) =====
(function(){
  ...
  var st={service:null,date:null,slot:null,employeeId:null,_slots:[],_services:[]};
  var view={y:0,m:0};
  var MES=['enero','febrero','marzo','abril','mayo','junio','julio','agosto','septiembre','octubre','noviembre','diciembre'];
  var css=[
   '#lz-book .lz-h{font-family:"Cormorant Garamond",serif;color:var(--forest);font-size:22px;margin:4px 0 12px}',
  ...
  root.innerHTML=
   ...
   '<div id="lz-step-svc"><label>1 · Elige servicio</label><select id="lz-svc"><option value="">Cargando servicios…</option></select><div class="lz-meta" id="lz-svcmeta"></div></div>'+
   '<div id="lz-step-date" hidden><label>2 · Elige día</label><div id="lz-cal"></div></div>'+
   '<div id="lz-step-time" hidden><label>3 · Elige hora</label><div id="lz-times"></div></div>'+
  ...
  api('/salon/'+SLUG).then(function(s){
    var svcs=(s&&s.services)||[]; st._services=svcs;
    g('lz-svc').innerHTML='<option value="">Elige un servicio…</option>'+svcs.map(function(v){return '<option value="'+v.id+'">'+esc(v.name)+'  ·  '+(v.durationMin||0)+' min  ·  '+eur(v.priceEur)+'</option>';}).join('');
  ...
  function loadTimes(ds){
    ...
    show('time'); g('lz-times').innerHTML='<div class="lz-msg">Buscando horas libres…</div>'; renderCal();
    api('/availability?salonSlug='+SLUG+'&date='+ds+'&serviceId='+st.service.id).then(function(a){
      var slots=(a&&a.slots)||[]; st._slots=slots;
      if(!slots.length){ g('lz-times').innerHTML='<div class="lz-msg">No hay horas libres ese día. Prueba con otro.</div>'; return; }
      g('lz-times').innerHTML=slots.map(function(s,i){return '<button type="button" class="lz-time" data-i="'+i+'">'+fmt(s.startsAt)+'</button>';}).join('');
    }).catch(function(){ g('lz-times').innerHTML='<div class="lz-msg">No se pudo cargar la disponibilidad. Prueba de nuevo.</div>'; });
  }
...
<section class="booking sec-pad" id="reservar"><div class="wrap">
  ...
  <div class="cal" id="lz-book" data-slug="luluca-nails-fuenlabrada" data-api="https://api.lanzo.es/api/public" data-wa="{c['whatsapp']}">
